...
| 数量上限 | `maxInMemory`（默认 3000），达到上限后 Producer 停止加载新任务，打印 WARN |
...
对于重试间隔很长的任务（如 `next_plan_time` 在 1 小时后），执行失败后从内存移除。Producer 每 `taskFindInterval` 扫描一次，在临近执行时间时自动加载。对长间隔任务，`taskFindInterval` 的加载延迟相对误差可忽略（3600s 中延迟 20s ≈ 0.6%）。
...
Producer 每次扫描的预加载窗口之间有重叠（`preloadWindow = taskFindInterval * preloadMultiplier >= taskFindInterval`），同一个任务可能被连续两次扫描都命中。`inMemoryTaskKeys` 天然防止了这情况：

```
扫描1 (T=0):   加载 next_plan_time <= T+40s 的任务 -> enqueue() -> addKey
扫描2 (T=20):  加载 next_plan_time <= T+60s 的任务
               -> 重叠区 [T+20, T+40] 中的任务再次被查到
               -> enqueue() -> inMemoryTaskKeys.add() 返回 false -> 跳过
扫描3 (T=40):  加载 next_plan_time <= T+80s 的任务
               -> 之前已入队的任务 next_plan_time 已小于 T+40，不在本轮窗口
               -> 新进入窗口的任务首次被加载
```
...
| 层级 | 机制 | 说明 |
|------|------|------|
| 入队去重 | `inMemoryTaskKeys` | 同一 key 只入队一次 |
| DB 状态过滤 | SQL 只查 `status IN (WAITING, FAIL)` | 执行中(RUNNING)和已完成(SUCCESS)的任务不会被扫描 |
| 执行前校验 | `validateTaskInDB()` | DelayQueue 出队后再次确认 DB 状态，防止时钟跳变等异常 |

#### 3.4.2 afterExecute 竞态窗口（良性）

`afterExecute()` 中 `inMemoryTaskKeys.remove(key)` 和 `enqueue(task)` 之间存在极短窗口，Producer 可能在此间隙通过 `enqueue()` 将同一任务加入 DelayQueue。结果是**良性的**——两个入队携带相同的 `next_plan_time`，实际只会执行一次（DB 状态校验 + ConsumerTask 执行后的去重清理保证）。
...
- Producer 下一次扫描（taskFindInterval 内）自动加载新分片的任务到 DelayQueue
- 无需额外处理，接管延迟 taskFindInterval 对于故障恢复可接受
...
| `scheduler.max-in-memory` | 3000 | 内存中最大任务数量 |
    ...
    public void run() {
        while (SmartRetryExit.isExit()) {
            ...
            for (RetryTask task : tasks) {
                enqueue(task);  // 仅入队，inMemoryTaskKeys 自动去重
                                 // 已存在的跳过，不重复提交
            }
...
| 风险 | 缓解措施 |
|------|---------|
| DelayQueue 中任务因 JVM 崩溃丢失 | Producer 兜底扫描在 taskFindInterval 内恢复 |
